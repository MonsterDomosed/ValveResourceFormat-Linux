...
/// <summary>
/// Theme-resolved colors for the node graph renderer. Chrome colors come from a host-supplied
/// <see cref="GraphChromeColors"/>, hue slots are explicit per-theme constants so the same
/// <see cref="GraphHue"/> reads correctly on both the dark and the light canvas.
/// </summary>
internal class GraphPalette
{
    ...
    public GraphPalette(bool isDark, GraphChromeColors chrome)
    {
        Canvas = chrome.AppMiddle;
        NodeBody = chrome.AppSoft;
        Text = chrome.Contrast;
        TextDim = chrome.ContrastSoft;
        GridDot = chrome.Border;
        Selection = chrome.Accent;
        SelectionSoft = chrome.Accent.WithAlpha(150);
        ...
        MessageText = isDark ? chrome.Accent : new SKColor(32, 96, 190);
