...
/// layout. Rendering lives in the Render partial. Hosted by the platform graph viewers.
/// </summary>
internal partial class GraphView : IDisposable
{
    ...
    {
        Palette = palette ?? GraphPalette.Default;
    }
    ...
    public void HandleMouseDown(SKPoint graphPoint, GraphMouseButton button, GraphModifiers modifiers)
    {
        using var _ = stateLock.EnterScope();
        ...
        if ((button & GraphMouseButton.Left) != 0)
        {
            if (hitNode == null && hitWire == null && modifiers != GraphModifiers.Shift)
            {
                ClearSelection();
            }
            ...
            {
                if (modifiers == GraphModifiers.Shift)
                {
    ...
    public void HandleMouseMove(SKPoint graphPoint, GraphModifiers modifiers = GraphModifiers.None)
    {
        using var _ = stateLock.EnterScope();
        ...
        if (IsMoving)
        {
            ...
            var moveAllConnected = (modifiers & GraphModifiers.Control) != 0;
    ...
    public void HandleMouseUp(SKPoint graphPoint, GraphMouseButton button)
    {
        using var _ = stateLock.EnterScope();
